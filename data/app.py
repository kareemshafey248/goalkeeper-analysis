...
profile_path = Path(__file__).parent / "profile.jpg"
if profile_path.exists():
    st.sidebar.image(str(profile_path), use_container_width=True)

st.sidebar.markdown("""
<div style="text-align: center; padding-bottom: 16px;">
    <div style="font-size: 1.3rem; font-weight: 800; color: #e8f4fd;">Kareem Elshafey</div>
    <div style="font-size: 0.95rem; color: #60a5fa; margin-bottom: 4px;">Football Analyst ⚽</div>
    <div style="font-size: 0.85rem; color: #9ca3af;">📱 +20 112 797 3132</div>
</div>
""", unsafe_allow_html=True)
...
if page == "🏆 World Rankings":
    ...
    for i, row in top5.iterrows():
        ...
        with cols[i]:
            # Generate an avatar for the player
            player_img = f"https://api.dicebear.com/7.x/initials/svg?seed={row['player'].replace(' ', '+')}&backgroundColor=0a1929&textColor=e8f4fd"
            
            st.markdown(f"""
            <div class="gk-card" style="border-color:{color}44; text-align:center;">
                <div style="display:flex; justify-content:center; align-items:center; position:relative; margin-bottom:12px;">
                    <img src="{player_img}" style="border-radius:50%; width:64px; height:64px; border:2px solid {color};">
                    <div style="position:absolute; top:-10px; right:10%; font-size:1.4rem; font-weight:900; color:{color}; text-shadow: 1px 1px 3px rgba(0,0,0,0.8);">#{rank}</div>
                </div>
                <div style="font-size:1.0rem; font-weight:700; color:#e8f4fd; margin:4px 0;">{row['player']}</div>
                ...
                <div style="font-size:0.72rem; color:#6b7280; margin-bottom:10px;" title="Overall rating out of 100 combining all stats">Overall Grade 🌟</div>
                ...
                <div style="display:flex; justify-content:space-between; font-size:0.75rem;">
                    <span style="color:#9ca3af;" title="Percentage of shots saved">Saves<br><b style="color:#e8f4fd; font-size:0.85rem;">{row['save_pct']:.0f}%</b></span>
                    <span style="color:#9ca3af;" title="Matches without conceding a goal (Clean Sheets)">Clean Sheets<br><b style="color:#e8f4fd; font-size:0.85rem;">{row['clean_sheet_pct']:.0f}%</b></span>
                    <span style="color:#9ca3af;" title="Extra goals prevented beyond expectation (PSxG)">Goals Prevented<br><b style="color:#e8f4fd; font-size:0.85rem;">+{row['psxg_diff']:.1f}</b></span>
                </div>
            </div>
            """, unsafe_allow_html=True)
    ...
    st.markdown("""
    <div style="background: rgba(37, 99, 235, 0.1); border-left: 4px solid #3b82f6; padding: 12px; border-radius: 4px; margin-bottom: 16px; font-size: 0.9rem; color: #bfdbfe;">
        <b>💡 How to read this table:</b> The <b>Score</b> is an overall grade (0-100) combining all their stats. <b>PSxG+/-</b> means how many <i>extra goals</i> they prevented that an average keeper would have let in.
    </div>
    """, unsafe_allow_html=True)
    ...
    display["Score"]  = display["Score"].round(0)
    display["Save %"] = display["Save %"].round(0)
    display["CS %"]   = display["CS %"].round(0)
    display["GA/Game"]= display["GA/Game"].round(2)
    display["PSxG+/-"]= display["PSxG+/-"].round(1)
    display["UCL Save %"] = display["UCL Save %"].round(0)
...
# ══════════════════════════════════════════════════════════════════
    ...
    with col_l:
        ...
        metrics_to_show = [
            ("Save %",      f"{row['save_pct']:.0f}%"),
            ("Clean Sheet %", f"{row['clean_sheet_pct']:.0f}%"),
            ("Goals/Game",  f"{row['goals_per_game']:.2f}"),
            ("PSxG (Goals Prevented)", f"+{row['psxg_diff']:.1f}"),
            ("UCL Save %",  f"{row['ucl_save_pct']:.0f}%"),
            ("Pen Save %",  f"{row['penalty_save_pct']:.0f}%"),
            ("Distribution",f"{row['pass_completion_pct']:.0f}%"),
        ]
    ...
    with col_r:
        ...
        # Fan-friendly insight generator
        fan_insight = f"{row['player']} is an elite goalkeeper for {row['squad']}. He successfully saves <b>{row['save_pct']:.0f}%</b> of the shots he faces, and has miraculously prevented <b>{row['psxg_diff']:.1f}</b> extra goals that an average goalkeeper would have conceded! On top of that, he keeps a clean sheet (no goals allowed) in <b>{row['clean_sheet_pct']:.0f}%</b> of his matches. A true wall in goal!"

        # Insight box
        st.markdown(f"""
        <div class="insight-box">
            <b style="color:#60a5fa;">📊 Analyst Insight (Explained for Fans)</b><br><br>
            {fan_insight}
        </div>""", unsafe_allow_html=True)
